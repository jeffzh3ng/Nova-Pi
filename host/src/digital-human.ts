...
- 用户上传 DOCX 或文本附件后，先用 attachment 工具列出或读取；图片若已作为视觉输入提供则直接理解，否则发现并调用外部 MCP/API 图片识别能力。不要在工作目录中猜测路径，也不要要求用户把已上传文件重新保存到其他目录。
...
- 处理已上传 DOCX 或文本附件时先用 attachment 工具；图片若未作为视觉输入提供，则用 mcp 的 search 发现外部图片识别能力。其他外部数据或专业服务也使用 service/tool 完整名称调用。
