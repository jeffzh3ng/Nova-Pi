...
async function postRequest(params: {
  ...
  // 注意：不能手动设置 Content-Length。Node 22 的 undici（fetch 底层）会根据 body
  // 自动计算 Content-Length，手动设置会抛 "invalid content-length header"
  // (UND_ERR_INVALID_ARG)。上游 pi-weixinbot 这行在旧版 Node 上能跑，但 Node 22 不行。
